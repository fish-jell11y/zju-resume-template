...
\pagenumbering{gobble}
 ...
 \begin{tikzpicture}[remember picture, overlay] 
     \node[anchor = north east] at ($(current page.north east)+(-2cm,-1.2cm)$) {\includegraphics[height=3cm]{微信图片_2026-05-07_161653_686.jpg}};
 \end{tikzpicture}%
 \begin{tikzpicture}[remember picture, overlay] 
...
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    \item \textbf{特征工程与奖牌预测}：基于运动员历史表现等10项特征构建 \textbf{随机森林 (Random Forest)} 模型，精准预测奥运会奖牌榜 ($R^2=0.87$)；并结合 \textbf{K-means聚类与卡方检验}，深挖各参赛国的优势体育项目。
    ...
    \item \textbf{核心产出}：完成并提交25页全英文数据驱动数模论文。
\end{itemize}

\projectsep

\textbf{代数曲线和模空间相关问题探讨 (院级SRTP项目) \quad 项目负责人} \hfill 2025.03 -- 至今 \titlebreak
项目描述：本项目旨在探索基础代数几何核心理论，并初步调研其在数据降维等应用数学领域的理论拓展。
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    \item \textbf{代数几何基础理论研究}：系统研读代数曲线与模空间相关专著，深入学习代数簇、层论、上同调理论等核心理论，构建扎实的纯数学理论框架。
    \item \textbf{降维理论的几何视点探索}：跨学科调研数据科学中的底层数理逻辑，重点剖析 Johnson-Lindenstrauss 引理；从纯理论角度探讨高维数据随机投影后的几何结构保持性，挖掘代数几何视角下的潜在应用价值。
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{专业技能：} 熟练使用 LaTeX 进行学术文档排版，具备良好的数据分析基础
\end{itemize}
\end{document}
